...
from audiocap.evaluation_tools.coco_caption.pycocotools.coco import COCO
from audiocap.evaluation_tools.coco_caption.pycocoevalcap.eval import COCOEvalCap
...
def evaluate_metrics(prediction_file: Union[str, Path, List[Dict[str, str]]],
    ...
    for file_name in file_names:
        assert(file_name in reference_dict) 
    #    )
    #)
    ...
    for row in prediction_file:
        ...
        cap_names = ['caption_{:d}'.format(i) for i in range(1, nb_reference_captions+1)]
...
@app.command()
def main(
    prediction_file: Path = typer.Option(..., dir_okay=False, file_okay=True, readable=True, help="Path to CSV predictions file"),
    reference_file: Path = typer.Option(..., dir_okay=False, file_okay=True, readable=True, help="Path to CSV reference file"),
) -> None:
    input("Wait for debugger")
    total_metrics, file_names = evaluate_metrics(prediction_file, reference_file)
    
    df = pd.DataFrame({'': ['Score']+file_names})
    
    for k,v in total_metrics.items():
        df[k] = [v['score']] + [v['scores'][f] for f in file_names]
    
    df.to_csv(f'eval_{reference_file.stem}.csv', index=False)
