...
def main(
    ...
    ds_clotho, ds_clotho_mini = get_clotho_dataset(clotho_dir,
                                                   tokenizer,
                                                   feature_extractor,
    ...
    callback_log_val_preds = audiocap.callbacks.WandbPredictionLogger(
        log_prefix="val",
        dataset=ds_clotho_mini["val_mini"],
        collator=collator,
    ...
    callback_log_dev_preds = audiocap.callbacks.WandbPredictionLogger(
        log_prefix="dev",
        dataset=ds_clotho_mini["dev_mini"],
        collator=collator,
        log_every_n_steps=log_preds_every_n_steps,
        generate_kwargs={"max_length": training_args_dict["generation_max_length"]},
    )
    ...
    trainer = transformers.Seq2SeqTrainer(
        ...
        train_dataset=ds_clotho["dev"],
        eval_dataset=ds_clotho["val"],
        args=training_args,
        callbacks=callbacks,
    )
    ...
    for split_name in ds_raw.keys():
        ...
        split = (ds_raw[split_name]
            ...
            .map(
                ...
                batched=True,
                batch_size=16,
                remove_columns=["audio", "source_ds", "task", "prefix"],
            )
        )
...
def get_audioset_dataset(audioset_dir: pathlib.Path,
    ...
    for split_name in ds_raw.keys():
        ...
        split = (ds_raw[split_name]
            .to_iterable_dataset()
            .map(
                audiocap.preprocess.audioset_set_columns,
                batched=True,
                batch_size=10,
            )
            .shuffle(
